...
# ======================================
# 仿照 QATushare.py 的数据获取方法, 客户端换成湘财 xcsc_tushare。
#
...
def QA_fetch_get_trade_date(exchange='SSE', start='19900101', end=None):
    """交易日历(湘财 trade_cal)"""
    ...
    if df is None or len(df) == 0:
        return pd.DataFrame()
    df['date'] = df['cal_date'].apply(
        lambda x: QA_util_date_int2str(int(str(x)[0:8])))
    return df
